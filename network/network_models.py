...
from network.network import C3F2, C3F2_REINFORCE_with_baseline, C3F2_ActorCriticShared, AlexNet, AlexNetNotInitialized
from numpy import linalg as LA
from tensorflow import keras
from keras.applications import MobileNet, MobileNetV3Small, EfficientNetB0
...
class initialize_network_DeepQLearning():
    def __init__(self, cfg, name, vehicle_name):
         ...
         # ►► NEW: remember cfg so helper can see it ◄◄
        self.cfg = cfg
        ...
        stat_writer_path = os.path.join(cfg.network_path, self.vehicle_name, 'return_plot')
        loss_writer_path = os.path.join(cfg.network_path, self.vehicle_name, f'loss_{name}')

        # Create TensorBoard writers
        self.stat_writer = tf.summary.create_file_writer(stat_writer_path)
        self.loss_writer = tf.summary.create_file_writer(loss_writer_path)
        ...
        self.optimizer = tf.keras.optimizers.Adam(learning_rate=cfg.learning_rate, beta_1=0.9, beta_2=0.99)
    ...
    def build_model(self, cfg):
        ...
        inputs = tf.keras.Input(shape=(cfg.input_size, cfg.input_size, 3),
                                name="rgb_frame")
        
        # Resize / standardise frame size
        # x = tf.image.resize(inputs, (224, 224))   # match backbone input
        # x = tf.image.resize(inputs, (227, 227))

        # --------------------------------------------------------------
        # 1) BACKBONE SELECTION  (MobileNetV3 / EfficientNet / etc.)
        #    choose imagenet weights or none
        # --------------------------------------------------------------
        backbone_name  = cfg.model_name          # "MobileNetV3" / "EfficientNet" / …
        pretrained_src = cfg.get("pretrained", "none").lower()   # add to cfg
        if pretrained_src not in ("imagenet", "none"):
            raise ValueError("pretrained must be 'imagenet' or 'none'")

        weights_flag   = "imagenet" if pretrained_src == "imagenet" else None

        if backbone_name == 'MobileNetV1':
            from keras.applications.mobilenet import preprocess_input
            preprocess = preprocess_input
            base_model = MobileNet(input_shape=(cfg.input_size,cfg.input_size,3),
                                   include_top=False,
                                   weights=weights_flag)
        elif backbone_name == 'MobileNetV3':
            from keras.applications.mobilenet_v3 import preprocess_input
            preprocess = preprocess_input
            base_model = MobileNetV3Small(input_shape=(cfg.input_size,cfg.input_size,3),
                                          include_top=False,
                                          weights=weights_flag)
        elif backbone_name == 'EfficientNet':
            from keras.applications.efficientnet import preprocess_input
            preprocess = preprocess_input
            base_model = EfficientNetB0(input_shape=(cfg.input_size,cfg.input_size,3),
                                        include_top=False,
                                        weights=weights_flag)
        else:
            raise ValueError(f"Unsupported model: {backbone_name}")

        # --------------------------------------------------------------
        # 2) OPTIONAL FREEZE of lower layers
        # --------------------------------------------------------------
        self._apply_freeze_policy(base_model, cfg.train_type, cfg.base_model_trainable)
        ...
        outputs = tf.keras.layers.Dense(cfg.num_actions, activation='linear')(x)
    ...
    # ------------------------------------------------------------------
    # helper: freeze policy
    # ------------------------------------------------------------------
    def _apply_freeze_policy(self, base_model, train_type: str, base_model_trainable):
        """
        train_type in cfg:
          - "e2e"     : train everything
          - "last4"   : freeze all except last 4 conv blocks
          - "last3", "last2"
        """
        ...
        if base_model_trainable == False:
            ...
            print(f"Back bone is not trainable")
            return

        if train_type == "e2e":
            base_model.trainable = True
            print(f"Train_type '{train_type}' - back bone is trainable")
            return

        if not train_type.startswith("last"):
            print(f"[WARN] Unknown train_type '{train_type}', defaulting to e2e")
            base_model.trainable = True
            return

        try:
            keep = int(train_type.replace("last", ""))
        except ValueError:
            print(f"[WARN] Bad train_type '{train_type}', defaulting to e2e")
            base_model.trainable = True
            return

        # freeze all but the last <keep> trainable layers
        for layer in base_model.layers[:-keep]:
            layer.trainable = False
        print(f"[Freeze] {len(base_model.layers)-keep} layers frozen, "
              f"{keep} trainable")

    # ------------------------------------------------------------------
    # load_network (unchanged except better error)
    # ------------------------------------------------------------------
    ...
    # ------------------------------------------------------------------
    # helper: forward pass for a batch of states (no gradient)
    # ------------------------------------------------------------------
    def Q_val(self, states):
        ...
        states : np.ndarray or tf.Tensor
                shape = (B, H, W, 3), already normalised to [0,255] or
                whatever preprocessing you expect.

        Returns
        -------
        np.ndarray of Q-values, shape = (B, cfg.num_actions)
        """
        if isinstance(states, np.ndarray):
            states = tf.convert_to_tensor(states, dtype=tf.float32)

        q_values = self.model(states, training=False)   # forward pass
        return q_values.numpy()   

    def log_to_tensorboard(self, tag, group, value, index):
        full_tag = f"{group}/{tag}"
        with self.stat_writer.as_default():
            tf.summary.scalar(name=full_tag, data=value, step=index)
            self.stat_writer.flush()
    ...
    def train_step(self, states, targets, actions):
        with tf.GradientTape() as tape:
            ...
            pred_q_values = tf.gather_nd(q_values, indices)
            # Compute loss
    ...
    def train_n(self, xs, ys, actions, batch_size, dropout_rate, lr, epsilon, iteration):
        # Update learning rate if necessary
        self.optimizer.learning_rate = lr

        # Perform a training step
        loss, q_values = self.train_step(xs, ys, actions)
        ...
        with self.stat_writer.as_default():
            ...
            tf.summary.scalar('Learning Rate', lr, step=iteration)
            tf.summary.scalar('Mean Q', mean_q, step=iteration)
            tf.summary.scalar('Max Q', max_q, step=iteration)
    ...
    def save_network(self, save_path, episode=''):
        save_path = os.path.join(save_path, self.vehicle_name, f'{self.vehicle_name}_{episode}')
