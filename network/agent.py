...
class PedraAgent():
    ...
    def get_state(self):
        ...
        camera_image = get_MonocularImageRGB(self.client, self.vehicle_name, self.input_size)
        self.raw_bgr = camera_image # keep image copy for ball detection using YOLO

        self.iter = self.iter + 1

        # Check if the camera image is valid before resizing
        if camera_image is not None and camera_image.size > 0:
            ...
            # print(f"state_rgb shape without expand_dims: {state_rgb.shape}, dtype: {state_rgb.dtype}")
        else:
            print("Failed to retrieve a valid camera image for resizing.")
            # Use a default zero image if camera_image is invalid
            state_rgb = np.zeros((self.input_size, self.input_size, 3), dtype=np.float32)
